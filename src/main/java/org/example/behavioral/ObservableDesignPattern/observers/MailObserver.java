...
public class MailObserver implements GenericObserver{
    @Override
    public void update(Product p, ProductObservable observable) {

        if(observable instanceof BackInStockObservable){
            System.out.println("Telling users via Mail that item " + p.getName() + " is back in stock"
                    + "with only " + p.getNumberOfItems() + " units available");

        } else if (observable instanceof OutOfStockObservable) {
            System.out.println("Telling users via Mail that item " + p.getName() + " is out of stock");
